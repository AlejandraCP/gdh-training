...
@Component({
  ...
  styleUrls: [
    "./crear-curso.component.css",
    "./../../view-user-home/view-user-home.component.css"
  ]
})
export class CrearCursoComponent implements OnInit {
  ...
  public hour: any;
  public minute: any;
  public initialAmPm: any;
  public finishAmPm: any;

  ngOnInit() {
    // get list change box teacher
    this.boolSelectExpoService
      .getBoolSelect()
      .snapshotChanges()
      .subscribe(item => {
        this.boolSelectList = [];
  ...
  show() {
    ...
    console.log(this.initialAmPm);
    console.log(this.finishAmPm);
  }
  ...
  resetFormCourse(courseForm?: NgForm) {
    if (courseForm != null) {
      courseForm.reset();
      this.misCursosService.newCourse = new CursoModel();
    }
  }
  ...
  onSubmit(teacherForm: NgForm) {
    if (teacherForm.value.$key == null) {
      ...
      this.counterService.updateTeacherId(
        this.counterList[0].$key,
        teacherForm.value.expositorId
      );
  ...
  onSubmitCourse(courseForm: NgForm) {
    if (courseForm.value.$key == null) {
      courseForm.value.cursoId = this.counterList[0].teacherId + 1;
      this.counterService.updateTeacherId(
        this.counterList[0].$key,
        courseForm.value.cursoId
      );
      courseForm.value.expositor_Id = 5;
      courseForm.value.horario = this.date + this.hour + this.minute;
      courseForm.value.imgUrl = 'imgUrl';
      courseForm.value.imgNombre = 'imgNombre';
      courseForm.value.nombreManual = 'nombreManual';
      courseForm.value.urlManual = 'urlManual';
      courseForm.value.nombreSilabus = 'nombreSilabus';
      courseForm.value.urlSilabus = 'urlSilabus';
      courseForm.value.tipoCurso_Id = 'tipocursoid';
      this.misCursosService.insertCourse(courseForm.value);
    } else {
      this.misCursosService.updateCourse(courseForm.value);
    }
    this.resetFormCourse(courseForm);
  }
}
